...
- Update the `lookup_keys_where` and `lookup_values_where` API documentation
  and README Quick Start for the current signatures: `ResultOrder` is now
  included before the value-output flags, and schema filtering uses
  `Some(MySchema::schema_params())`.
- Document lookup criteria as ANDed exact-match filters, the ascending default
  ordering, and that ordering is applied before zero-based `Limit` pagination.

## [0.4.1] - 2026-08-15

- Add `ResultOrder` and optional ordering arguments to key-range, bulk, and lookup reads.
